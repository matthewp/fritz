...
function createElement$1 (node) {
  ...
  if (childNodes) {
    childNodes.forEach(ch => realNode.appendChild(render$2(ch)));
  }
...
function createText$1 (el) {
  return document.createTextNode(el.textContent);
}

function render$2 (node) {
  if (node instanceof Node) {
    return node;
  }
  if (Array.isArray(node)) {
    const frag = document.createDocumentFragment();
    node.forEach(item => frag.appendChild(render$2(item)));
    return frag;
  }
  const realNode = node.localName ? createElement$1(node) : createText$1(node);
  nodeMap[node.__id] = realNode;
  return realNode;
}

var mount = function (tree, elem) {
...
var appendChild = function (src, tar) {
  nodeMap[src.__id].appendChild(render$2(tar));
};

var removeChild$1 = function (src, tar) {
...
var removeAttribute = function (src, tar, data) {
  const { name } = data;
  nodeMap[src.__id].removeAttribute(name);
};

var replaceChild = function (src, tar) {
  const realSrc = nodeMap[src.__id];
  if (realSrc) {
    realSrc.parentNode && realSrc.parentNode.replaceChild(render$2(tar), realSrc);
  } else {
    src.__id = tar.__id;
...
var setAttribute = function (src, tar, data) {
  const { name } = data;
  nodeMap[src.__id].setAttribute(name, tar.attributes[name]);
};

var setEvent = function (src, tar, data) {
  const realSrc = nodeMap[src.__id];
...
var setProperty = function (src, tar, data) {
  const { name } = data;
...
patchers[SET_PROPERTY] = setProperty;
patchers[TEXT_CONTENT] = textContent;

function patch$1 (instruction) {
...
var patch$2 = function (instructions) {
  instructions.forEach(patch$1);
};
